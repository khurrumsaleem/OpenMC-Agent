...
<img src="assets/vera3b_xz_combined.png" alt="VERA3B xz combined" style="width:100%; max-width:700px;"/>
</div>

**图：** VERA3B 单组件 xz 切面（由 OpenMC ``openmc -p`` 渲染；图为横置展示）。从左到右对应轴向 $z\in[-55, 463.937]$ cm：两端分别为下反射层与上 nozzle/plenum，中间约 365 cm 的 active fuel 段（$z\in[11.951, 377.711]$ cm）与 8 个 spacer grid band 的材料分层清晰可见。左半按 material 染色，右半按 cell 染色；端塞路径、气腔与燃料段的层间过渡符合 VERA3B 几何契约。
...
<div style="flex:1; min-width:240px; text-align:center;">
<img src="assets/vera4_xy_material.png" alt="VERA4 xy material" style="width:100%;"/>
<sub>(a) material 视图</sub>
</div>
<div style="flex:1; min-width:240px; text-align:center;">
<img src="assets/vera4_xy_cell.png" alt="VERA4 xy cell" style="width:100%;"/>
<sub>(b) cell 视图</sub>
</div>
</div>

**图：** VERA4 3×3 全堆芯 xy 切面（活性燃料中平面，由 OpenMC ``openmc -p`` 在 ``data/runs/phase8c_step3n_vera4_redraw_v1/`` 渲染）。9 个组件（center / edge / corner 三类）位置与装载符合 VERA4 几何契约；center 组件中的 RCCA 局部插入段亦清晰可见。本运行使用 deterministic source-backed fixture 离线组装（不调 LLM），``renderability=runnable``，XML export 成功。

<div style="display:flex; gap:8px; flex-wrap:wrap; margin:1em 0;">
<div style="flex:1; min-width:240px; text-align:center;">
<img src="assets/vera4_xz_material.png" alt="VERA4 xz material" style="width:100%;"/>
<sub>(a) material 视图</sub>
</div>
<div style="flex:1; min-width:240px; text-align:center;">
<img src="assets/vera4_xz_cell.png" alt="VERA4 xz cell" style="width:100%;"/>
<sub>(b) cell 视图</sub>
</div>
</div>

**图：** VERA4 3×3 全堆芯 xz 轴向切面（由 OpenMC ``openmc -p`` 渲染）。轴向可见活性燃料段、气腔、端塞与上/下结构界面；本 plan 显式使用径向 reflective、轴向 vacuum 六面边界（与 VERA4 benchmark 物理边界一致）。本运行同上 xy 图，为 deterministic fixture 离线组装的渲染结果。
