...
补充：外部 openmc-smoke seed v1 已完成真实 OpenMC 路径（`FIRST_PASS_SUCCESS`、`real_openmc_verified=true`、`xml_exported=true`、`llm_call_count=0`），但 campaign aggregate status 因只识别 `*PASSED*` disposition 被误标 failed。现聚合逻辑显式接受 `FIRST_PASS_SUCCESS`/`RECOVERED_SUCCESS`；focused status tests `48 passed`，全量非 OpenMC/非 LLM pytest `3768 passed, 2 skipped, 392 deselected`，`compileall`、fake benchmark `21/21` 通过。平台内本地 v2 复跑被 UCX `getifaddrs error: Operation not permitted` 阻断，需以外部命令行 smoke 作为正式验证面。
