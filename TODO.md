...
当前基线：`main`，已闭合 v20 fuel/coolant stray `source_variant_id`（非 fuel variant binding 清理 + MU fuel-only coverage/duplicate/mismatch）、Facts 驱动毒物 universe 隔离（monolithic/fragmented/state 全路径）与 component-profile loading 自动附着（end_plug/plenum/gas_gap/shoulder_gap，方向消歧）；保留既有 Pyrex annular-poison validator 安全边界。
全量测试：非 OpenMC/非 LLM `3890 passed, 2 skipped, 393 deselected`；`compileall` 通过；fake workflow benchmark `21/21`；baseline diff 因 baseline 文件缺失跳过
里程碑：v20 实际 Facts+Materials+Universes 已 valid，但 MU 被 fuel/coolant 共用短 variant label 阻塞；保存三件套在新 normalizer/preflight 下离线回放为 **MU ok=True, 0 issues**。
下一步：用 SenseNova 托管 `ds:deepseek-v4-flash` 跑 VERA3B v21 全链路，验证 MU→Placement→Axial→Assembled；若失败，优先抽取新 gate artifact 离线闭合。
