...
当前基线：`main`，最近确认生产修复：MaterialsPatch source-declared coolant boron normalization；Pyrex annular poison / upper helium plenum deterministic validation；selected benchmark state fuel-variant scope；Facts reviewer normalization firewall；Facts source-note schema-boundary closure；Facts downstream-detail owner boundary；Facts required-coverage revision path closure；Facts JSON Patch/schema/schema-out metadata adapter hardening；Facts whole-source evidence reconciliation（chunk-local unsupported finding 降级）；localized-insert 详细几何 downstream-owner 路由；确定性 annular-insert universe oracle（pyrex_rod LLM 失败回退）；localized-insert gas-plenum alias materialization（absorber→gas 变换）；patch JSON dropped-object-opener 兜底修复；Facts multi-segment insert profile contract 放宽（`required_segment_roles` 等价满足）；Materials redundant compound-component 去重；离线 campaign closure 回归 suite（`tests/fixtures/offline_closure/` + `test_offline_campaign_closure.py`，7 个真实失败脱敏 fixture，无 LLM 回放）；Materials 碎片化生成激活修复（缺 inventory set 时从 facts 派生需求集；auto 先单体再碎片化；executor 始终走 materials pipeline）；Materials fuel source_variant_id 规范化（短标签→Facts canonical variant_id，歧义 fail-closed）；单体 materials 规范化补传 state（fuel 规范化在生成期即触发）；universes fuel_variant 校验容忍 canonical-equivalent 短标签；large-lattice 检测修复小数维度误报（21.50×21.50 不再误判为 50）；Facts reviewer 拒绝证据/metadata 路径 finding；MU preflight 用 classify_species_name 区分核素/compound（pyrex B10 不再误报）；v19 上游黄金基线 fixture；UniversesPatch 隐式 universe 归一化（implicit_* satellite cell role 修正 + radial-layer 合并 + 半径兼容性检查 + 径向连续性调整）
全量测试：非 OpenMC/非 LLM `3883 passed, 2 skipped, 393 deselected`；`compileall` 通过；fake workflow benchmark `21/21`；baseline diff 因 baseline 文件缺失跳过
里程碑：真实 LLM run（VERA3B v19）首次 facts+materials+universes 三上游 gate 全部 valid，**material_universe 下游 gate MU ok=True**（仅剩 `background_missing` warning，非 blocking）
下一优先级：fuel_pin universe `background_missing`（warning）——燃料棒只有 fuel+gap+clad，缺 moderator 背景 cell；可考虑（A）加强 fuel_pin validator 容忍 lattice-based 模型无 background cell，或（B）确定性 fuel-pin 构造 oracle 自动补齐 background
...
1. 重新从当前生产路径跑 VERA3B 与 VERA4 base；旧五 Gate seed 已不再代表当前 Pyrex validator 合同，不能作为最终通过依据；
