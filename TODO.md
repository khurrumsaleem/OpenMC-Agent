...
全量测试：非 OpenMC/非 LLM `3856 passed, 2 skipped, 393 deselected`；`compileall` 通过；fake workflow benchmark `21/21`；baseline diff 因 baseline 文件缺失跳过
仍开放（无确定性修复，待结构性对策）：materials 大 patch 输出截断（提高 PATCH_MAX_TOKENS 或启用 materials 碎片化）；materials 输出碎片化（多段 prose-separated JSON，需解析器合并或 prompt 收敛）
