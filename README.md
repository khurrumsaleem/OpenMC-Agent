...
- **Phase 8C MU deterministic material checks**: MaterialsPatch validation, materials fragment qualification, and MU preflight now reject `atom_frac`/`weight_frac` compositions whose sums indicate unnormalized stoichiometric ratios (for example water `H1=2, O16=1`). Partial fraction vectors (`<=1`) and percent-style vectors (`≈100`) remain valid; chemical ratios must declare `stoichiometric_ratio`.
- **Phase 8C shared insert-profile coverage**: inventory-driven universe requirements preserve all localized insert requirement IDs served by a shared radial profile. Fragmented Universes metadata records both the legacy singular ID and the complete ID set, and MU preflight can also recover coverage from `GeometryComponentInventory` for old checkpoints.
